...
export function Gallery({
  ...
  if (useFlexibleGrid) {
    desktopView = <FlexibleGrid photos={photos} spans={spans!} editing={editing} />;
  } else if (photos.length === 2) {
    desktopView = <PairAsymmetric photos={photos} editing={editing} />;
  } else if (photos.length === 3) {
    desktopView = <HeroPlusTwo photos={photos} editing={editing} />;
  } else if (photos.length === 4) {
    desktopView = <Bento4 photos={photos} editing={editing} />;
  } else {
    desktopView = <Masonry photos={photos} editing={editing} />;
  }
  ...
  const mobileView = useFlexibleGrid ? (
    <FlexibleGrid photos={photos} spans={spans!} editing={editing} mobile />
  ) : (
    <Masonry photos={photos} editing={editing} />
  );
...
function PairAsymmetric({
  ...
  return (
    <div className="grid aspect-[5/2] grid-cols-[3fr_2fr] gap-3">
      <Tile photo={photos[0]} sizes="432px" editing={editing} idx={0} />
      <Tile photo={photos[1]} sizes="288px" editing={editing} idx={1} />
    </div>
  );
}
...
function HeroPlusTwo({
  ...
  return (
    <div className="grid aspect-[3/2] grid-cols-[3fr_2fr] grid-rows-2 gap-3">
      ...
      <div className="relative">
        <Tile photo={photos[2]} sizes="288px" editing={editing} idx={2} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="grid aspect-[4/5] grid-cols-3 grid-rows-3 gap-3">
      <div className="relative col-span-2 row-span-2">
        <Tile photo={photos[0]} sizes="480px" editing={editing} idx={0} />
      </div>
      <div className="relative">
        <Tile photo={photos[1]} sizes="240px" editing={editing} idx={1} />
      </div>
      <div className="relative">
        <Tile photo={photos[2]} sizes="240px" editing={editing} idx={2} />
      </div>
      <div className="relative col-span-3">
        <Tile photo={photos[3]} sizes="720px" editing={editing} idx={3} />
      </div>
    </div>
  );
}
    ...
    <div className="gap-3 [column-count:2] sm:[column-count:3]">
      {photos.map((p, i) => {
        ...
        return (
          <div
            key={`${p.filePath}-${i}`}
            className="group/photo relative mb-3 overflow-hidden rounded-lg bg-zinc-100 break-inside-avoid dark:bg-zinc-900"
    ...
    >
      {photos.map((p, i) => {
        ...
        return (
          <div
            key={`${p.filePath}-${i}`}
            className="group/photo relative overflow-hidden rounded-lg bg-zinc-100 dark:bg-zinc-900"
            style={{
